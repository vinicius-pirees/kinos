...
class TestOneClassSVMPrediction(unittest.TestCase):
    ...
    def test_oneclass_svm_with_pca(self):

        oneclass_svm = OneClassSVM()
        oneclass_svm.fit(self.training_data_acquirer.data)
        ...
        for msg in self.inference_data_acquisition.consumer.consumer:
            ...
            if sequence_counter == sequence_size:
                ...
                prediciton = oneclass_svm.predict(sequence)
                print(prediciton)
                sequence_counter = 0
                sequence = []
